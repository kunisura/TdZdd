 ...
 * $Id: CnfBdd.hpp 518 2014-03-18 05:29:23Z iwashita $
 */
...
class CnfBdd: public ScalarDdSpec<CnfBdd,CnfBddState> {
    typedef CnfBddState::ClauseNumber ClauseNumber;
    typedef CnfBddState::ClauseSet ClauseSet;
    ...
    //MyVector<DdStructure> frontierZdd; // reachable state set for each level
...
//                    bdd2zdd(frontierSet[i], frontierClauses[i]));
//            frontierSet[i] = 0;
//        }
...
//        NodeId g = frontierZdd[level].root();
//        NodeTableEntity<2> const& diagram = *frontierZdd[level].getDiagram();
//
//        for (typeof(clauses.rbegin()) t = clauses.rbegin(); t != clauses.rend();
//                ++t) {
//            while (g.row() > int(*t)) { // 0
//                g = diagram.child(g, 0);
//            }
//
//            if (g.row() < int(*t)) return true;
//
//            assert(g.row() == int(*t)); // 1
//            g = diagram.child(g, 1);
//        }
//
//        return g == 0;
